...
/* Example logic for the main loop (obviously will be different but gives a decent idea of the flow of logic)

bool curve_generated = false; // I would put this in the global space

PIDController steeringPID(400.0, 0.0, 50.0); // just some sample values, integral should start at 0 though

// This goes in the void loop() 

if (PROBE_RELEASE && !curve_generated) {
    p0 = gps_to_meters(live_lat, live_lon); // This is the location where we release
    if ((get_distance(p0, gate1_pos) < get_distance(p0, gate2_pos)) {
        p1 = gate1_pos; 
    } else {
        p1 = gate2_pos;
    }
...
// This sets the initial gate upon release (can be changed by pass refrence automatically if we get blown off course considerably)

if (curve_generated) {
    
    // 1. Where are we right NOW?
    Point2D current_pos = gps_to_meters(live_lat, live_lon);

    // 2. Did the wind blow us off course? (Uses live position)
    check_and_redraw_path(current_pos, p0, p1, p2, gate1_pos, gate2_pos, 20.0);

    // 3. Which way do we steer? (Uses live position)
    double raw error = calculate_steering_error(current_pos, heading_rad, p0, p1, p2);

    double servo_correction = steeringPID.compute(raw_error);

    leftServo.writeMicroseconds(1500 - servo_correction);
    rightServo.writeMicroseconds(1500 + servo_correction);
    
    // ^ Includes PID, if we don't want to use PID then literally just dont call the PID struct func and use the calculate_steering_error() func
}

// This is like pretty much all of it in terms of 2D steering, now we just have to consider changing altitude for egg release along the path

*/

// add descent controller next